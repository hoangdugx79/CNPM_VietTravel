  ...
  return (
    <AdminLayout title="Lịch khởi hành" subtitle="Quản lý đợt mở bán, giá và sức chứa theo tour">
      ...
      <div className="admin-card"><div className="card-body">
        <div className="data-controls"><form className="admin-filter-form" onSubmit={(event) => event.preventDefault()}><CustomSelect className="filter-select" value={filters.tourId} onChange={(event) => setFilters((current) => ({ ...current, tourId: event.target.value, page: 1 }))} options={[{ value: '', label: 'Tất cả tour' }, ...tours.map((item) => ({ value: item.TourId, label: item.Title }))]} placeholder="Tất cả tour" /></form></div>
      ...
      <AdminModal open={formOpen} title={editingItem ? 'Cập nhật lịch khởi hành' : 'Thêm lịch khởi hành'} onClose={() => setFormOpen(false)} footer={<><button type="button" className="btn btn-outline" onClick={() => setFormOpen(false)} disabled={submitting}>Đóng</button><button type="submit" form="departure-form" className="btn btn-primary" disabled={submitting}>{submitting ? 'Đang lưu...' : 'Lưu'}</button></>}>
        <form id="departure-form" onSubmit={handleSubmit}>
          {!editingItem ? <div className="form-group"><label className="form-label">Tour</label><CustomSelect className="admin-select" value={form.tourId} onChange={(event) => setForm((current) => ({ ...current, tourId: event.target.value }))} options={[{ value: '', label: 'Chọn tour' }, ...tours.map((item) => ({ value: item.TourId, label: item.Title }))]} placeholder="Chọn tour" /></div> : null}
          ...
          <div className="form-row"><div className="form-group"><label className="form-label">Giá em bé</label><input className="form-control" type="number" min="0" value={form.infantPrice} onChange={(event) => setForm((current) => ({ ...current, infantPrice: event.target.value }))} /></div><div className="form-group"><label className="form-label">Trạng thái</label><CustomSelect className="admin-select" value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value }))} options={[{ value: 'open', label: 'Mở bán' }, { value: 'closed', label: 'Đóng bán' }, { value: 'cancelled', label: 'Đã hủy' }]} /></div></div>
        </form>
      </AdminModal>
