...
export default function DashboardCharts({ revenueByMonth = [], topTours = [] }) {
  ...
  const topTourLabels = topTours.map((tour) => tour.Title || 'Chưa đặt tên');
    ...
    <div className="admin-dashboard-grid">
      <section className="admin-card admin-dashboard-card admin-dashboard-wide">
        <div className="card-header">
          <div>
            <div className="card-title"><i className="fas fa-chart-line" /> Doanh thu 6 tháng gần đây</div>
            <p className="admin-dashboard-subtitle">Theo dõi doanh thu và lượng booking trên dashboard admin.</p>
          </div>
        </div>
        <div className="card-body">
          {revenueByMonth.length > 0 ? (
            ...
            <div className="admin-dashboard-empty">Chưa có dữ liệu doanh thu để vẽ biểu đồ.</div>
          )}
        </div>
      </section>
      ...
      <section className="admin-card admin-dashboard-card">
        <div className="card-header">
          <div>
            <div className="card-title"><i className="fas fa-chart-pie" /> Top tour nổi bật</div>
            <p className="admin-dashboard-subtitle">Tỷ trọng booking của các tour được đặt nhiều nhất.</p>
          </div>
        </div>
        <div className="card-body">
          {topTours.length > 0 ? (
            ...
            <div className="admin-dashboard-empty">Chưa có dữ liệu top tour để hiển thị.</div>
          )}
        </div>
      </section>
    </div>
